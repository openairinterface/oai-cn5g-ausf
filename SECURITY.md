...
- **Email subject**: `[SEC-VUL-OAI-CN5G-AUSF]: "Mention the affected
  functionality here"`.
...
in the
[GitHub Issue Tracker](https://github.com/openairinterface/oai-cn5g-ausf/issues),
...
- **Issue title**: `[AI-SEC-VUL-OAI-CN5G-AUSF]: "Mention the affected
  functionality here"`.
- **Issue label**: apply the
  https://github.com/openairinterface/oai-cn5g-ausf/labels/security
...
The [openairinterface/oai-cn5g-ausf](https://github.com/openairinterface/oai-cn5g-ausf)
repository contains the OAI implementation of the 5G Core **Authentication
Server Function (AUSF)**, together with its configuration templates, container
images, deployment artifacts, and CI tooling. Only the 5G Core is in scope in
this repository.

Issues in the other OAI CN5G network functions (AMF, SMF, UPF, UDM, UDR, NRF,
...
can be reported here if they are reachable through the AUSF. In that case,
please state clearly which component is affected and give the commit ID of the
affected submodule in addition to the AUSF commit.

Security reports are in scope when they affect the confidentiality, integrity,
or availability of the AUSF running in a documented or reasonably expected
deployment.
...
- Memory corruption, crashes, infinite loops, resource exhaustion, or denial of
  ...
  requests on N12 or responses on N13, or by malformed YAML in the
  configuration file parsed at startup.
- Defects in the implemented 5G-AKA flow (the UE authentication initiation and
  the 5G-AKA confirmation operations) that allow authentication to succeed with
  an incorrect or replayed RES*, allow the confirmation step to be skipped or
  reordered, or allow an authentication context to be reused across UEs
  (3GPP TS 33.501).
- Defects in key handling and derivation, including the computation of HXRES*
  from XRES* and of KSEAF from KAUSF, and the cryptographic primitives in
  [src/5gaka/](./src/5gaka), where a weakness could let a serving network
  obtain key material it is not entitled to or let an attacker predict derived
  keys.
- Defects that allow a consumer to confirm or retrieve the authentication of a
  subscriber it did not initiate, for example through forged, guessable, or
  reused authentication context identifiers.
- Exposure of subscriber identifiers such as the SUPI or SUCI, of
  authentication vectors, or of key material such as KAUSF, KSEAF, RES*, or
  XRES*, through logs, statistics, API responses, or error paths.
...
- Bugs requiring local admin/root access on the AUSF host with no privilege
  boundary crossed.
...
- Vulnerabilities solely caused by third-party projects or dependencies should
  ...
  the AUSF deployment or requires a change in this repository, please report it
  here as well.
- Attacks that require a trusted operator role, such as an already-authorised
  AMF or UDM deliberately misconfigured by the deployment owner, unless a
...
- Denial of service that only depends on flooding a plaintext, unprotected
  ...
  deployment, rather than on an AUSF software defect.
