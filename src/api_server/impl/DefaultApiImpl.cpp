...
void DefaultApiImpl::ue_authentications_auth_ctx_id5g_aka_confirmation_put(
  ...
  } else  // AV valid
  {
    ...
    } else  // success
    {
      ...
      udmUri = "http://" + udm_ip + ":" + udm_port + "/nudm-ueau/v1/" +
  ...
  Logger::ausf_server().debug(
      ...
      confirmResponse_json.dump().c_str());
...
void DefaultApiImpl::ue_authentications_post(
  ...
  Logger::ausf_server().info(
      "supiOrSuci %s", authenticationInfo.getSupiOrSuci().c_str());
  ...
  // UDM POST interface ----- get authentication related
  // info--------------------
  udmUri = "http://" + udm_ip + ":" + udm_port + "/nudm-ueau/v1/" + supi +
  ...
  resourceURI = "http://" + ausf_ip + ":" + ausf_port +
                "/nausf-auth/v1/ue-authentications/" + authCtxId_s +
                "/5g-aka-confirmation";
  ausf_Href.setHref(
